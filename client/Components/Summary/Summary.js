  ...
  const fetchAttendanceReport = async () => {
    try {
      const respone = await axios.get(
        `http://192.168.191.218:3000/attendance-report-all-employees`,
        {
          params: {
            month: 11,
            year: 2023,
  ...
  return (
    <ScrollView style={{flex: 1, backgroundColor: 'white'}}>
      <View
        ...
        <Icon2 onPress={goToPrevMonth} name="left" size={24} color="black" />
        <Text>{formatDate(currentDate)}</Text>
        <Icon2 onPress={goToNextMonth} name="right" size={24} color="black" />
      </View>
